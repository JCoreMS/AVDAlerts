...
param ScriptsRepositoryUri string = 'https://raw.githubusercontent.com/JCoreMS/AVDAlerts/main/deployEnterprise/scripts/'

@description('The Resource Group ID for the AVD session host VMs in the same region.')
param SessionHostsResourceGroupIds array = [
  ''
]

@description('The Resource IDs for the Azure Files Storage Accounts used for FSLogix profile storage in the same region.')
param StorageAccountResourceIds array = [
  ''
]

@description('The Resource IDs for the Azure NetApp Volumes used for FSLogix profile storage in the same region.')
...
module mod_subscription_AVDAlertsResources './subAVDAlerts.bicep' = {
  ...
  params: {
    ...
    ResourceGroupName: ((!empty(UserResourceGroup))) ? UserResourceGroup : ResourceGroupName
...
// Assign Automation Account LAW Contributor
module mod_roleAssignments_LAWSub './modules/assignRoleLAW.bicep' = {
  name: 'roleAssign_LAWSub'
  scope: subscription(split(LogAnalyticsWorkspaceResourceId,'/')[2])
  params: {
    DeployToSub: DeployToSub
    LogAnalyticsWorkspaceResourceId: LogAnalyticsWorkspaceResourceId
    RoleAssignments: RoleAssignments
    PrincipalId: mod_subscription_AVDAlertsResources.outputs.automationAccountPrincipalId

  }
}
// Assign Automation Account VM Reader on all AVD Subs
module mod_roleAssignment_AVD_VMs './modules/assignRoleVMs.bicep' = [for Sub in AVDHostSubIDs: {
  name: 'roleAssign_VMSub'
...
// Assign Automation Account Store Account Reader
module mod_roleAssignment_AzStorAcct './modules/assignRoleStorAcct.bicep' = [for Sub in AzStorAcctSubIDs: {
  name: 'roleAssign_AzStorAcct'
  scope: subscription(Sub)
  params: {
    CurrentSub: Sub
    RoleAssignments: RoleAssignments
