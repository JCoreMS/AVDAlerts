...
// Assign LAW Contributor
resource roleAssignment_LAWSub 'Microsoft.Authorization/roleAssignments@2022-04-01' = {
  name: guid(subscription().id, RoleAssignments.StoreKeyRead.Name)
  properties: {
    principalId: resources.outputs.automationAccountPrincipalId
    roleDefinitionId: resourceId(split(LogAnalyticsWorkspaceResourceId,'/')[2],'Microsoft.Authorization/roleDefinition', RoleAssignments.StoreKeyRead.GUID)
    principalType: 'ServicePrincipal'
  }
}

resource roleAssignment_AVD_VMs 'Microsoft.Authorization/roleAssignments@2022-04-01' = {
  name: guid(subscription().id, RoleAssignments.DesktopVirtualizationRead.Name)
   properties: {
    principalId: resources.outputs.automationAccountPrincipalId
    roleDefinitionId: resourceId('Microsoft.Authorization/roleDefinition', RoleAssignments.DesktopVirtualizationRead.GUID)
    principalType: 'ServicePrincipal'
  }
}
...
output AVDSubHostIDs array = AVDHostSubIDs 
